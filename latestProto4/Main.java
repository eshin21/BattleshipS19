				...
				for(Button b : gameButtons) {
					if(b.inButton(point)) {
						...
						if(b.type.equals("Reset A")) {
						    ...
						    if(getMyGame().armada_A.size()==5 && currentShip.player.equals("A") && getMyGame().moveCount == 1) {
						        
						        for(Ship s : getMyGame().armada_A) {
                                    if(s.checkArmadaOverlap()) {
                                      this.overlapMessage = true;
						...
						if(getMyGame().armada_B.size()==5 && currentShip.player.equals("B") && getMyGame().moveCount == 2) {
						    ...
						    else if(getMyGame().moveCount == 0 || getMyGame().moveCount == 2){
						        getMyGame().moveCount++;
...
//                 g.fillRect((int)currentShipB.position.x, (int) currentShipB.position.y, currentShipB.xdim, currentShipB.ydim);
		...
		if (myGame.moveCount==1){ // first player places ships
			...
			if (onePlayerMode){
				...
				if (checkedMove){
					g.setColor(Color.YELLOW);
					g2d.drawString("Please select a weapon first!", 50, 17);                           g.fillRect((int)targetPoint.x+5,(int)targetPoint.y+5,40,40);
                    if (choseWeapon){       
                        g.setColor(Color.YELLOW);       
	...
	public void keyPressed(KeyEvent e) {
        ...
        if(c=='r') {
            ...
            if(currentShip.player.equals("A") && currentShipA != null) {
                ...
                if(!rotated.isOOB(rotated.position)) {
